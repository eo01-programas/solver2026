function renderBalanceModule() {
    ...
    const summaryPureGroups = filterGroupsByIds(pureGroups, crudoIds);
    const summaryMixGroups = filterGroupsByIds(mixGroups, crudoIds);
    ...
    const summaryPure = generateSummaryTable(summaryPureGroups, false, {
        title: 'Resumen de totales (Solo Crudos)',
        subtitle: 'Kg requerido · Crudos',
        isCrudo: true,
        excludeMix: true
    });
    document.getElementById('bal-pure-summary-area').innerHTML = summaryPure ? `<div class="summary-block" style="margin-top:20px;">${summaryPure}</div>` : '';

    const summaryMix = generateSummaryTable(summaryMixGroups, false, {
        title: 'Resumen de totales (Solo Mezclas)',
        subtitle: 'Kg requerido · Mezclas',
        isCrudo: true
    });
    document.getElementById('bal-mix-summary-area').innerHTML = summaryMix ? `<div class="summary-block" style="margin-top:20px;">${summaryMix}</div>` : '';

    // El resumen HTR se muestra en RESUMEN TOTALES y tambien en la pestana HTR
    const summaryHtrTab = generateSummaryTable(summaryHtrGroups, true, {
        title: 'Resumen de totales (Solo HTR)',
        subtitle: 'Kg requerido · HTR',
        isCrudo: false
    });
    document.getElementById('bal-htr-summary-area').innerHTML = summaryHtrTab ? `<div class="summary-block" style="margin-top:20px;">${summaryHtrTab}</div>` : '';
    ...
    function collectFromGroups(groups, isHtr) {
        ...
        allItems.forEach(r => {
            ...
            if (!isMezcla) {
                ...
                if (options && options.excludeMix) return;
                const comps = isHtr ? parseMixCompositionHtr(r.hilado || "", r.kg, { groupName: r.group }) : parseMixComposition(r.hilado || "", r.kg, { groupName: r.group });
                comps.forEach(c => {
                    const mapped = mapComponentToBase(c.name, crudoKeys);
                    const target = mapped || c.name;
